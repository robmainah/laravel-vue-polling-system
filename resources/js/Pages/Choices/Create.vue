...
const createChoice = async () => {
    const storeData = await axios.post(`/questions/${props.question.id}/choices`, form.value)
    if (storeData.status === 201) {
        router.get(`/questions/${props.question.id}/choices`);
    }
};
</script>

<template>
    <Head title="Create New Choice" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Create New Choice</h2>
        </template>
